...
async def get_tsn_piece_by_id(db: Session, id: int):
    return db.query(db_models.TsnPiece).filter(db_models.TsnPiece.id == id).first()
...
async def edit_tsn_piece(db: Session, tsn_piece: schemas.TsnPieceEdit):  # двойная запись?
    ...
    new_piece = db.query(db_models.TsnPiece).filter(db_models.TsnPiece.id == tsn_piece.id).first()
...
async def get_sn_piece_by_id(db: Session, id: int):
    return db.query(db_models.SnPiece).filter(db_models.SnPiece.id == id).first()
...
async def edit_sn_piece(db: Session, sn_piece: schemas.SnPieceEdit):  # двойная запись?
    ...
    new_piece = db.query(db_models.SnPiece).filter(db_models.SnPiece.id == sn_piece.id).first()
    if new_piece is None:
        return None
    db.commit()
    return new_piece
...
async def edit_spgz_piece(db: Session, spgz_piece: schemas.SpgzPieceEdit):
    update = {k: v for k, v in spgz_piece.dict().items() if v is not None}
    if "kpgz_piece_id" in update:
        if await get_kpgz_piece_by_id(db, update["kpgz_piece_id"]) is None:
            return None
    del update["id"]
    db.query(db_models.SpgzPiece).filter(db_models.SpgzPiece.id == spgz_piece.id).update(update)
    new_piece = db.query(db_models.SpgzPiece).filter(db_models.SpgzPiece.id == spgz_piece.id).first()
    if new_piece is None:
        return None
    db.commit()
    return new_piece
...
async def edit_kpgz_piece(db: Session, kpgz_piece: schemas.TsnPieceEdit):  # двойная запись?
    ...
    new_piece = db.query(db_models.KpgzPiece).filter(db_models.KpgzPiece.id == kpgz_piece.id).first()
    if new_piece is None:
        return None
    db.commit()
    return new_piece
